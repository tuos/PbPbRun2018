void trkETT_plot(){
   ...
   TFile *inFile = new TFile("../../matched_tree.root");
   TNtuple* tree1 = (TNtuple*)inFile->Get("evtMatchedTree");
   //TTree *tree1 = (TTree*)inFile->Get("HLTinfo/HLTeff");
   Int_t trk, trkoffline;
   Double_t ett, trkp, trkm, vz;
   tree1->SetBranchAddress("ETT",&ett);
   tree1->SetBranchAddress("NtrkFull",&trk);
   tree1->SetBranchAddress("NtrkOffline",&trkoffline);
   ...
   for (Long64_t i=0;i<nentries;i++) {
     ...
     //hETTntrk->Fill(trkoffline, ett);
     if(trk==0 || trkoffline==0) continue;
     hETTntrk->Fill(trkoffline, trk);
  }
  ...
  //tree1->Draw("ETT:NtrkOffline>>hprof","NtrkFull<200&&ETT<200","goff");
  //tree1->Draw("NtrkFull:NtrkOffline>>hprof","NtrkFull<200&&ETT<200&&NtrkFull>0&&trkoffline>0","goff");
  tree1->Draw("NtrkFull:NtrkOffline>>hprof","NtrkFull<200&&ETT<200&&NtrkFull>0&&NtrkOffline>0","goff");
...
//cout<<"k = "<<k<<"  mx: "<<px->GetMean()<<endl;
  ...
  htmp->GetYaxis()->SetTitle("NtrkOnline");
  htmp->GetXaxis()->SetTitle("NtrkOffline");
