...
echo -ne `dasgoclient --query="file dataset=/HIForward/HIRun2018A-v1/RAW | grep file.nevents" | awk 'BEGIN{sum=0} {sum+=$1} END {print sum}' 2>/dev/null`"\t"
...
echo -ne `dasgoclient --query="file dataset=/HIHeavyFlavor/HIRun2018A-v1/RAW | grep file.nevents" | awk 'BEGIN{sum=0} {sum+=$1} END {print sum}' 2>/dev/null`"\t"
...
echo -ne `dasgoclient --query="file dataset=/HIMinimumBiasReducedFormat0/HIRun2018A-v1/RAW | grep file.nevents" | awk 'BEGIN{sum=0} {sum+=$1} END {print sum}' 2>/dev/null`"\t"
echo ""
